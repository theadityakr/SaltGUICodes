...
function xUserLockOut {
    if ((Get-WMIObject win32_computersystem).partofdomain -eq $false){
        ExecuteFunction -RegKeyName UserLockOut -FunctionName $function:UserLockOut -IntervalInMins 720
    }
  }
      ...
      function NewLocalUserCheck {
    $Days = 1
    $result = Get-WinEvent -FilterHashtable @{LogName="Security";Id=4720} -MaxEvents 100 | Where-Object {($_.TimeCreated -ge (Get-Date).AddDays(-$Days))} | ForEach-Object {
        ...
        If($evt.PrimaryGroupId -like '512'){
            $IsAdmin = 'Yes'
        }Else{
            $IsAdmin = 'No'
        }
        $xevt = [ordered]@{
            EventDate = $EventDate
            NewUser = $evt.TargetUserName
            CreatedBy = $evt.SubjectUserName
            AdminRights = $IsAdmin
        }
        [PsCustomObject]$xevt
    }
    ...
    If($result -ne $null){
        $Message =  $result
        $Body =  $VMINFO + "`r`n" + $Message
        $Subject = "New user local user created on $Env:ComputerName"
        Send-MailMessage -From 'adchange@apps4rent.com' -To $EmailTo -Subject $Subject -Body $body -SmtpServer $SMTPServer
    }
}
...
$PD = "VwAzACEAYwAwAG0AZQBAADIAQAAxADkAJABeAEAAIwAmAA=="
  ...
  function xNewLocalUserCheck {
    ExecuteFunction -RegKeyName UserLockOut -FunctionName $function:NewLocalUserCheck -IntervalInMins 1440
  }

              xUserLockOut        #UserLockOut Settings enabled

      xNewLocalUserCheck  # Detects local user created in last 24hrs.