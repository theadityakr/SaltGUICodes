...
function ScanNotePad {
$xNotepad = Get-ChildItem -Path C:\ -Force | Where-Object {$_.Name -eq 'DontDeleteThis.txt'}
If($xNotepad -eq $null)
{
    $Filename = Get-ChildItem -Path C:\ -Force | Where-Object {$_.Name -like 'DontDeleteThis*'}
    $Message =  "File extension is changed to $($Filename.FullName)"
    $Body =  $VMINFO + "`r`n" + $Message
    $Subject = "Possible Virus on: $Env:ComputerName"
...
function xScanNotePad {
    ExecuteFunction -RegKeyName ScanNotePad -FunctionName $function:ScanNotePad -IntervalInMins 5
}


    xScanNotePad     # This scans the notepad extensions for possible virus. Email Subject: Possible Virus on: