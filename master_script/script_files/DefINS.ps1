...
function DefINS {
    ...
    If($Null -eq $AV){
        $ScriptBlock = {
            ...
            $MD = "C:\Windows\System32\Sysprep\Apps4Rent\"
            If(Test-Path $MD){
                Remove-item "$MD\Client" -Recurse -Force -Confirm:$false -ErrorAction SilentlyContinue | Out-Null
                Copy-Item "$DL\Client" -Destination "$MD\Client" -Recurse -Force -Confirm:$false | Out-Null
                } Else {
                Copy-Item "$DL\Client" -Destination "$MD\Client" -Recurse -Force -Confirm:$false | Out-Null
            }
            net use $DL /delete
            $f = Get-Item $MD -Force
            $f.attributes="Hidden"
            Start-Process cmd -ArgumentList "C:\Windows\System32\Sysprep\Apps4Rent\Client\scepinstall.exe /s /q /NoSigsUpdateAtInitialExp /policy %~dp0EPAMPolicy2.xml" -WindowStyle Hidden
            Start-Sleep -Seconds 300
            $AV = get-wmiobject -query "select * from AntiMalwareHealthStatus" -namespace "root\Microsoft\SecurityClient" -ErrorAction SilentlyContinue -ErrorVariable ERR
            If($Null -eq $AV){
                $Body =  $VMINFO  + "`r`n" + "Defender Installation Faile. Kindly review the logs and take required action."  + "`r`n"
                $Subject = "Windows Defender is not installed On: $Env:ComputerName"
...
function xDefINS {
    ExecuteFunction -RegKeyName DefINS -FunctionName $function:DefINS -IntervalInMins 1440
}

    #xDefINS          # This verify if Defender is installed or not. Email Subject: Windows Defender is not installed On: