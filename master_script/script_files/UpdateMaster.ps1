...
function UpdateMaster{
  ...
  If(Test-Path "C:\Windows\System32\Sysprep\Apps4Rent\A4RMaster.ps1"){
    $NMS = Get-ChildItem -Path $DL -Recurse | Where-Object {$_.FullName -like '*A4RMaster*'} | Sort-Object -Property LastAccessTime -Descending | Select-Object -First 1
    IF((Compare-Object $(Get-Content $MS) -DifferenceObject $(Get-Content $NMS.FullName))){
      Copy-Item -Path $NMS.FullName -Destination "C:\Windows\System32\Sysprep\Apps4Rent\A4RMaster.ps1" -Force -Confirm:$false | Out-Null 
    }
  } Else {
    $NMS = Get-ChildItem -Path $DL -Recurse | Where-Object {$_.FullName -like '*A4RMaster*'} | Sort-Object -Property LastAccessTime -Descending | Select-Object -First 1
    Copy-Item -Path $NMS.FullName -Destination "C:\Windows\System32\Sysprep\Apps4Rent\A4RMaster.ps1" -Force -Confirm:$false | Out-Null
...
function xUpdateMaster {
  ExecuteFunction -RegKeyName UpdateMaster -FunctionName $function:UpdateMaster -IntervalInMins 720
}



    xUpdateMaster    # This updates A4Rmaster Script.