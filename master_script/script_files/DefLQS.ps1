    ...
    If($AV.LastQuickScanAge -like "*2*"){
        ...
        if([int]$OSVersion.Major -ge 10){
            ...
            If($OS.name -like "*2016*"){
                $T = Get-Date
                Start-Process "C:\Program Files\Windows Defender\MSASCui.exe" -ArgumentList "-QuickScan -hide"  -WindowStyle Hidden
                $i = 0
                while ($i -le 10) {
                    Start-Sleep -Seconds 30
                    $Deflog = Get-WinEvent -LogName 'Microsoft-Windows-Windows Defender/Operational' | Where-Object {$_.timecreated -ge $T }
                    foreach($D in $Deflog){
                        if(($D.ID -eq 1001) -and ($D.TimeCreated -ge $T)){
                            $i+= 11
                        } Elseif(($D.ID -eq 1002) -and ($D.TimeCreated -ge $T)){
                            $output = "Yes"
                            $ISError = "Yes"
                            $xBody += "Scan has been cancelled." + "`r`n" + $D.Message + "`r`n"
                            $i+= 11
                        } Elseif (($D.ID -eq 1003) -and ($D.TimeCreated -ge $T)) {
                            $output = "Yes"
                            $ISError = "Yes"
                            $xBody += "Scan has been Paused." + "`r`n" + $D.Message + "`r`n"
                            $i+= 11
                        } Elseif (($D.ID -eq 1005) -and ($D.TimeCreated -ge $T)){
                            $output = "Yes"
                            $ISError = "Yes"
                            $xBody += "Scan has Failed." + "`r`n" + $D.Message + "`r`n"
                            $i+= 11
                        }
                    }
                }
                if (($i -ge 10) -and ($ISError -eq "No")){
                    $output = "Yes"
                    $xBody += "Quick Scan started at $T but didn't finish within 5mins" + "`r`n"
                }
            } Else {
                $T = Get-Date
                Start-Process "C:\Program Files\Windows Defender\MpCmdRun.exe" -ArgumentList "-Scan -ScanType 1"  -WindowStyle Hidden
                $i = 0
                while ($i -le 10) {
                    Start-Sleep -Seconds 30
                    $Deflog = Get-WinEvent -LogName 'Microsoft-Windows-Windows Defender/Operational' | Where-Object {$_.timecreated -ge $T }
            ...
            $T = Get-Date
                Start-Process "C:\Program Files\Microsoft Security Client\MpCmdRun.exe" -ArgumentList "-Scan -ScanType 1" -WindowStyle Hidden
                $i = 0
                while ($i -le 10) {
                    Start-Sleep -Seconds 30
                    $Deflog = Get-WinEvent -LogName 'System' | Where-Object {$_.timecreated -ge $T }
                    foreach($D in $Deflog){
                        if(($D.ID -eq 1001) -and ($D.TimeCreated -ge $T)){
                            $i+= 11
                        } Elseif(($D.ID -eq 1002) -and ($D.TimeCreated -ge $T)){
            ...
            if($output -like "*Yes*"){
                ...
                $Subject = "Windows Defender Last QuickScan is 2days old On: $Env:ComputerName"
...
function xDefLQS {
    ExecuteFunction -RegKeyName DefLQS -FunctionName $function:DefLQS -IntervalInMins 1440
}


    #xDefLQS          # This Verify the Defender QuicScan Age. Email Subject: Windows Defender Last QuickScan is 2days old On: