...
function DefRTP {
    ...
    If($AV.RtpEnabled -like "*False*"){
      $Body =  $VMINFO
      $Subject = "Windows Defender Real-time Protection is Disabled On: $Env:ComputerName"
      if ((Get-WMIObject win32_computersystem).partofdomain -eq $true){
        $Domain = (Get-WmiObject Win32_ComputerSystem).Domain
        if(($Domain -notlike "*Apps4Rent*") -and ($Domain -notlike "*Dataoncloud*") -and ($Domain -notlike "*Hostingcloudapp*") -and ($Domain -notlike "*citrix*") -and ($Domain -notlike "*ourhosted*")){
...
function xDefRTP {
    ExecuteFunction -RegKeyName DefRTP -FunctionName $function:DefRTP -IntervalInMins 720
}


    #xDefRTP          # This Verify the Defender Real-time Protection. Email Subject: Windows Defender Real-time Protection is Disabled On: 