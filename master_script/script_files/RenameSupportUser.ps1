...
Function RenameSupportUser {
    $PSVersion = $PSVersionTable.PSVersion.Major
    if($PSVersion -ge "5"){
    Write-Host "Renaming using Rename-LocalUser"
    Rename-LocalUser -Name "VDIsupport" -NewName "Summer" | Out-Null
    Rename-LocalUser -Name "Nocadmin" -NewName "Spring" | Out-Null
    } Else {
    Write-Host "Renaming User"
    $myAccount = Get-WmiObject Win32_UserAccount -Filter "name='VDIsupport'"
    $myAccount.Rename('Summer')
    $myAccount = Get-WmiObject Win32_UserAccount -Filter "name='Nocadmin'"
    $myAccount.Rename('Spring')
    }
...
$PD = "VwAzACEAYwAwAG0AZQBAADIAQAAxADkAJABeAEAAIwAmAA=="
  ...
  function xRenameSupportUser {
    ExecuteFunction -RegKeyName RenameSupportUser -FunctionName $function:RenameSupportUser -IntervalInMins 1440
  }


      xRenameSupportUser #Added to rename Support Accounts