...
Function UpdateRDPCertificate {
    Set-ExecutionPolicy -ExecutionPolicy RemoteSigned -Force -Confirm:$false
    $OS = (Get-WmiObject -class Win32_OperatingSystem).Caption
    If($OS -like "*2019*"){
        Add-MpPreference -ControlledFolderAccessAllowedApplications "C:\Windows\System32\WindowsPowerShell\v1.0\powershell.exe" -Force | Out-Null
    }
    ...
    If(!(Test-Path "$ENV:SystemDrive\Windows\Apps4Rent\Softwares\Cert")){
        New-Item -ItemType "directory" -Path "$ENV:SystemDrive\Windows\Apps4Rent\Softwares\Cert" -Force | Out-Null
    }
    $CurrentTP = (Get-WmiObject -class "Win32_TSGeneralSetting" -Namespace root\cimv2\terminalservices -Filter "TerminalName='RDP-tcp'").SSLCertificateSHA1Hash
    [string]$UN = 'WakeUpCall'
    [string]$PD = "VwAzACEAYwAwAG0AZQBAADIAQAAxADkAJABeAEAAIwAmAA=="
    [SecureString]$xPD = [System.Text.Encoding]::Unicode.GetString([System.Convert]::FromBase64String($PD)) | ConvertTo-SecureString -AsPlainText -Force 
    [PSCredential]$xCred = New-Object System.Management.Automation.PSCredential -ArgumentList $UN, $xPD

    $OldTPSource = 'http://ftp.globalhostedcloud.com/VDICerts/OldThumprint.txt'
    $OldTP = Invoke-RestMethod -Uri $OldTPSource  -Credential $xCred
    $NewTPSource = 'http://ftp.globalhostedcloud.com/VDICerts/NewThumprint.txt'
    $NewTP = Invoke-RestMethod -Uri $NewTPSource  -Credential $xCred
    function SetRDPCert{
        Param (
            [Parameter()]
            [ValidateNotNull()]
        ...
        $source = 'http://ftp.globalhostedcloud.com/VDICerts/vdi.hostingcloudapp.com.pfx'
        $destination = "$ENV:SystemDrive\Windows\Apps4Rent\Softwares\Cert\vdi.hostingcloudapp.com.pfx"
        $ProgressPreference = 'SilentlyContinue'
        Invoke-RestMethod -ContentType "application/x-pkcs12" -Uri $source -OutFile $destination -Credential $Credential | Out-Null
        $mypwd = ConvertTo-SecureString -String "apps4rent" -Force -AsPlainText
        $CertImport = Import-PfxCertificate $destination -CertStoreLocation "cert:\localMachine\my" -Password $mypwd -Exportable -Confirm:$false -ErrorAction SilentlyContinue
        $CertImport = Import-PfxCertificate $destination -CertStoreLocation "cert:\localMachine\Remote Desktop" -Password $mypwd -Exportable -Confirm:$false -ErrorAction SilentlyContinue
        $certThumbprint = $CertImport.Thumbprint
        $path = (Get-WmiObject -class "Win32_TSGeneralSetting" -Namespace root\cimv2\terminalservices -Filter "TerminalName='RDP-tcp'").__path
        Set-WmiInstance -Path $path -argument @{SSLCertificateSHA1Hash="$certThumbprint"} | Out-Null  
        $Hash = (Get-WmiObject -class "Win32_TSGeneralSetting" -Namespace root\cimv2\terminalservices -Filter "TerminalName='RDP-tcp'").SSLCertificateSHA1Hash  
        If($Hash -eq $certThumbprint){
            Write-Output "Cert Updated"
        } else {
            Write-Output "Cert Not Updated"
        }
        ...
        try{
            ...
            # Get Location of the machine related keys
                ...
                $acl.SetAccessRule($buildAcl) | Out-Null #Add Access Rule
                Set-Acl $keyFullPath $acl -ErrorAction SilentlyContinue | Out-Null #Save Access Rules
            }
        }catch{
    ...
    if ((Get-WMIObject win32_computersystem).partofdomain -eq $true){
        ...
        if($Domain -like "*VDI.Hostingcloudapp.com*" ){
            If($CurrentTP -ne $NewTP){
                SetRDPCert -Credential $xCred
            }
        }
    } Else {
        If ($CurrentTP -eq $OldTP){
            SetRDPCert -Credential $xCred
        }
    }
...
$PD = "VwAzACEAYwAwAG0AZQBAADIAQAAxADkAJABeAEAAIwAmAA=="
  ...
  function xUpdateRDPCertificate {
    ExecuteFunction -RegKeyName UpdateRDPCertificate -FunctionName $function:UpdateRDPCertificate -IntervalInMins 1440
  }


      xUpdateRDPCertificate # Update SSL Cert for RDP