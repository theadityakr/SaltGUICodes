...
function HiddenNIC {
    ...
    $GhostNIC = Get-WmiObject win32_networkadapter | Where-Object {($null -eq $_.ServiceName) -and ($userException -notcontains $_.Name)}
    If ($Null -ne $GhostNIC){
      $Body = $VMINFO + "`r`n" + [string]$GhostNIC.Name
      $Subject = "Hidden NIC Found: $Env:ComputerName"
...
$PD = "VwAzACEAYwAwAG0AZQBAADIAQAAxADkAJABeAEAAIwAmAA=="
  ...
  function xHiddenNIC {
    ExecuteFunction -RegKeyName HiddenNIC -FunctionName $function:HiddenNIC -IntervalInMins 1440
}

    xHiddenNIC       # This alerts on Hidden NIC's. Email Subject: Hidden NIC Found: