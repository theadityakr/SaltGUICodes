...
$PD = "VwAzACEAYwAwAG0AZQBAADIAQAAxADkAJABeAEAAIwAmAA=="
  ...
  function xRestartPrintSpooler {
    $min = Get-Date '01:00'
    $max = Get-Date '06:00'
    $now = Get-Date
    if ($min.TimeOfDay -le $now.TimeOfDay -and $max.TimeOfDay -ge $now.TimeOfDay) {
        ExecuteFunction -RegKeyName RestartPrintSpooler -FunctionName $function:RestartPrintSpooler -IntervalInMins 240
    }
  }


      xRestartPrintSpooler #Restart Print Spooler Service.