...
function SCANSMB1 {
    ...
    if($PSVersion -le '2'){
      $SMBv1Reg = (Get-Item HKLM:\SYSTEM\CurrentControlSet\Services\LanmanServer\Parameters | ForEach-Object {Get-ItemProperty $_.pspath}).SMB1  
      If($SMBv1Reg -like "*1*") {
        ...
        $MSG = "SMBv1 has been disabled and System Restart may required."
        $Body = $VMINFO +  "`r`n" + $MSG
        $Subject = "SMBv1 is Enabled On: $Env:ComputerName"
        Send-MailMessage -From $EmailFrom -To $EmailTo -Subject $Subject -Body $Body -SmtpServer $SMTPServer
      } 
    } else {
      $SMBv1Reg = (Get-Item HKLM:\SYSTEM\CurrentControlSet\Services\LanmanServer\Parameters | ForEach-Object {Get-ItemProperty $_.pspath}).SMB1
      $SMBv1 = (Get-SmbServerConfiguration -ErrorAction SilentlyContinue -ErrorVariable ERR ).EnableSMB1Protocol
      If($SMBv1 -like "*True*"){
...
function xSCANSMB1 {
    ExecuteFunction -RegKeyName SCANSMB1 -FunctionName $function:SCANSMB1 -IntervalInMins 1440
}

    xSCANSMB1        # This alerts on SMBv1. Email Subject: SMBv1 is Enabled On: