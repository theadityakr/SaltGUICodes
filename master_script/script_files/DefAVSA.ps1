...
function   ExceptionFunction {
    ...
    Return $KeysArray
            ...
            function DefAVSA {
$AV = get-wmiobject -query "select * from AntiMalwareHealthStatus" -namespace "root\Microsoft\SecurityClient" -ErrorAction SilentlyContinue -ErrorVariable ERR
If($AV.AntivirusSignatureAge -ge "2"){
    $ScriptBlock = {
        #Email Settings
        ...
        $IsRunning = "No"
            ...
            If($Winupdate -eq 'Running'){
                $IsRunning = "Yes"
            } Else {
                $IsRunning = "No"
                If($Winupdate.StartType -eq 'Disable') {
                    $IsDisable = "Yes"
                    Set-Service wuauserv -StartupType Manual -Confirm:$false | Out-Null
                    Start-Service wuauserv -Confirm:$false | Out-Null
                } Else {
                    Start-Service wuauserv -Confirm:$false | Out-Null
                }
            }
        ...
        if([int]$OSVersion.Major -ge 10){
            $OS = Get-WMIObject win32_operatingsystem |Select-Object Name
            If($OS.name -like "*2016*"){
                $CMD = cmd  /c "C:\Program Files\Windows Defender\MSASCui.exe" -Update -hide
                If(($CMD -like '*failed*') -or ($CMD -like '*ERROR*')){
                    $ISError = "Yes"
                    $xBody += $CMD
                }
            } Else {
                $CMD = cmd  /c "C:\Program Files\Windows Defender\MpCmdRun.exe" -SignatureUpdate
                If(($CMD -like '*failed*') -or ($CMD -like '*ERROR*')){
                    $ISError = "Yes"
                    $xBody += $CMD
                }
            ...
            $CMD = cmd  /c "C:\Program Files\Microsoft Security Client\MpCmdRun.exe" -SignatureUpdate
            If(($CMD -like '*failed*') -or ($CMD -like '*ERROR*')){
                $ISError = "Yes"
                $xBody += $CMD
            }
        ...
        if($ISError -like "*Yes*"){
            $Body =  $VMINFO +  "`r`n" +  "Failed to update Windows Defender Antivirus Signature." + "`r`n" + $xBody
            $Subject = "Windows Defender Antivirus Signature is 7days old On: $Env:ComputerName"
            Send-MailMessage -From $EmailFrom -To $EmailTo -Subject $Subject -Body $Body -SmtpServer $SMTPServer
            }
            $Test = Test-Path -Path "C:\Windows\mpam-feX64.exe"
            If($Test -eq 'True'){
                Remove-Item -LiteralPath "C:\Windows\mpam-feX64.exe" -Force | Out-Null
            }
            $Job = Start-BitsTransfer -Source "https://go.microsoft.com/fwlink/?LinkID=87341" -Destination "C:\Windows\mpam-feX64.exe" -Asynchronous 
            while (($Job.JobState -eq "Transferring") -or ($Job.JobState -eq "Connecting")) `
                { Start-Sleep 5;} # Poll for status, sleep for 5 seconds, or perform an action.
            Switch($Job.JobState)
            {
            ...
            Set-Service wuauserv -StartupType Manual -Confirm:$false | Out-Null
            Start-Service wuauserv -Confirm:$false | Out-Null
            Start-Process "C:\Windows\mpam-feX64.exe" -ArgumentList "-q" -NoNewWindow -Wait -ErrorAction SilentlyContinue | Out-Null
            $xAV = get-wmiobject -query "select * from AntiMalwareHealthStatus" -namespace "root\Microsoft\SecurityClient" -ErrorAction SilentlyContinue -ErrorVariable ERR
            Write-Output "Current Defination Age is $($xAV.AntivirusSignatureAge) | Old: $($AV.AntivirusSignatureAge)"
            Stop-Service wuauserv -Confirm:$false -Force | Out-Null
            Set-Service wuauserv -StartupType Manual -Confirm:$false | Out-Null
            $Test = Test-Path -Path "C:\Windows\mpam-feX64.exe"
            If($Test -eq 'True'){
            Remove-Item -LiteralPath "C:\Windows\mpam-feX64.exe" -Force | Out-Null
        ...
        If($IsDisable -eq 'Yes'){
            Stop-Service wuauserv -Confirm:$false -Force | Out-Null
            Set-Service wuauserv -StartupType Disable -Confirm:$false | Out-Null
        }
        #>
        Stop-Service wuauserv -Confirm:$false -Force | Out-Null
        Set-Service wuauserv -StartupType manual -Confirm:$false | Out-Null
...
function xDefAVSA {
    ...
    If($Date.DayOfWeek -eq "Monday"){
        ExecuteFunction -RegKeyName DefAVSA -FunctionName $function:DefAVSA -IntervalInMins 720
    } Elseif ($Date.DayOfWeek -eq "Tuesday"){
        ExecuteFunction -RegKeyName DefAVSA -FunctionName $function:DefAVSA -IntervalInMins 720
    }
    #>
    ExecuteFunction -RegKeyName DefAVSA -FunctionName $function:DefAVSA -IntervalInMins 1440
}




    #xDefAVSA         # This Verify the Defender Antivirus Signature Age. Email Subject: Windows Defender Antivirus Signature is 7days old On: