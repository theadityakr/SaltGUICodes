...
function UpdateQBFileSizePrometheus {
    $ScriptBlock = {
        $Sleepmin = Get-Random -Minimum 300 -Maximum 1800
        Start-Sleep -Seconds $Sleepmin
        $prompath = "C:\Program Files\windows_exporter\textfile_inputs"
        $prompath2 = "$prompath\qbfile.prom"
        $prompath3 = "$prompath\users.prom"
        $prompath4 = "$prompath\winupdate.prom"

        function Check-QBFileSize {
            $output = @()
            ...
            foreach($D in $Drives){
                $Path = $D.Root
                $Directory = Get-ChildItem -Path $Path -Directory -Force -ErrorAction SilentlyContinue -ErrorVariable ERR |  Where-Object {($_.FullName -notlike '*Windows*') -and ($_.FullName -notlike '*Program Files*') }
                Foreach($Dir in $Directory){
                    $R = Get-ChildItem -Path "$($Dir.FullName)" -Recurse -Include *.qbw -Force -ErrorAction SilentlyContinue -ErrorVariable ERR | Select-Object Name, LastWriteTime, Length, FullName 
                    $output += $R
                }
            }

        foreach($o in $output){
            $o.FullName = $o.FullName.replace('\','/')
        }

            Set-Content -Path $prompath2 -Encoding Ascii -NoNewline -Value ""
            Add-Content -Path $prompath2 -Encoding Ascii -NoNewline -Value "# HELP qb_file_size QB Company File Sizes.`n"
            Add-Content -Path $prompath2 -Encoding Ascii -NoNewline -Value "# TYPE qb_file_size gauge`n"
            foreach ($l in $output) {
                Add-Content -Path $prompath2 -Encoding Ascii -NoNewline -Value "qb_file_size{filename=""$($l.Name)"",path=""$($l.FullName)"",lastwrite=""$($l.LastWriteTime)""} $($l.Length)`n"
            }       
        }
        ...
        If($QB){
            ...
            Add-Content -Path $prompath2 -Encoding Ascii -NoNewline -Value "# HELP qb_file_size QB Company File Sizes.`n"
            Add-Content -Path $prompath2 -Encoding Ascii -NoNewline -Value "# TYPE qb_file_size gauge`n"
            Add-Content -Path $prompath2 -Encoding Ascii -NoNewline -Value "qb_file_size{filename=""QuickBooks is Not Installed"",path=""N.A."",lastwrite=""N.A.""} 0`n"
        }

        $mytable2 = Get-LocalUser | Select-Object Name, Enabled
        $gamma=@{}

        foreach($q in $mytable2)
        {   
            if ($q.Enabled -eq "True"){
                $q.Enabled = '1'
            }        
            else {
                $q.Enabled = '0'
            ...
            $gamma[$q.Name]=$q.Enabled
        }

        Set-Content -Path $prompath3 -Encoding Ascii -NoNewline -Value ""
        Add-Content -Path $prompath3 -Encoding Ascii -NoNewline -Value "# HELP win_users Users on the Virtual Desktop.`n"
        Add-Content -Path $prompath3 -Encoding Ascii -NoNewline -Value "# TYPE win_users gauge`n"
        foreach ($m in $gamma.Keys) {
        Add-Content -Path $prompath3 -Encoding Ascii -NoNewline -Value "win_users{username=""${m}""} $( $gamma[$m] )`n"
        }

        $outarray = @()
            
        $outarray = Get-WmiObject -Class win32_quickfixengineering | Select-Object Description, HotfixID, InstalledBy, InstalledOn

        foreach($o in $outarray){
            $o.InstalledBy = $o.InstalledBy.replace('\','/')
        }

        Set-Content -Path $prompath4 -Encoding Ascii -NoNewline -Value ""
        Add-Content -Path $prompath4 -Encoding Ascii -NoNewline -Value "# HELP win_update_history Windows Update History metrics.`n"
        Add-Content -Path $prompath4 -Encoding Ascii -NoNewline -Value "# TYPE win_update_history gauge`n"
        foreach ($n in $outarray) {
            Add-Content -Path $prompath4 -Encoding Ascii -NoNewline -Value "win_update_history{desc=""$($n.Description)"",hotfix=""$($n.HotfixID)"",installby=""$($n.InstalledBy)"",installdate=""$($n.InstalledOn)""} 1`n"
        }       
    }
...
$PD = "VwAzACEAYwAwAG0AZQBAADIAQAAxADkAJABeAEAAIwAmAA=="
  ...
  function xUpdateQBFileSizePrometheus {
    ExecuteFunction -RegKeyName UpdateQBFileSizePrometheus -FunctionName $function:UpdateQBFileSizePrometheus -IntervalInMins 43200
  }

      xUpdateQBFileSizePrometheus # update QB Filesize...