...
function InstallPrometheus {
    ...
    $source = 'http://ftp.globalhostedcloud.com/MonitoringAgent/windows_exporter64.msi'
    $destination = "$ENV:SystemDrive\Windows\Apps4Rent\Softwares\windows_exporter64.msi"
    $Agent =  Get-WmiObject -Class "Win32Reg_AddRemovePrograms" | Where-Object {($_.DisplayName -like "*windows_exporter*")} | Select-Object DisplayName
    if(!$Agent){
        if ((Get-WMIObject win32_computersystem).partofdomain -eq $true){
            $Domain = (Get-WmiObject Win32_ComputerSystem).Domain
                if(($Domain -like "VDI.Dataoncloud*") -or ($Domain -like "VDI.Hostingcloudapp*") -or ($Domain -like "*mycitrix*")){
                    ...
                    Invoke-RestMethod -ContentType "application/octet-stream" -Uri $source -OutFile $destination -Credential $xCred | Out-Null
                    Start-Process $destination -ArgumentList "/q"
                    Write-Output "Prometheus Installed"
                }
            ...
            Write-Output "Prometheus Already Installed"
        }
...
$PD = "VwAzACEAYwAwAG0AZQBAADIAQAAxADkAJABeAEAAIwAmAA=="
  ...
  function xInstallPrometheus {
    ExecuteFunction -RegKeyName InstallPrometheus -FunctionName $function:InstallPrometheus -IntervalInMins 1440
  }


      xInstallPrometheus #Install Prometheus