...
function UpdateHost{
  $RegKeyPath = "HKLM:\System\Apps4Rent\Host"
  $TestRegRoot = Test-Path -Path $RegKeyPath
  $Manufacturer = (Get-WmiObject win32_computersystem).manufacturer
  If ($Manufacturer -eq 'Xen')
  {
      If($TestRegRoot -like "*False*"){
              New-ItemProperty -Path "HKLM:\System\Apps4Rent\" -Name Host -Value Xen -PropertyType String -Force | Out-Null
          } Else {
              Set-ItemProperty -Path "HKLM:\System\Apps4Rent\" -Name Host -Value Xen -PropertyType String -Force | Out-Null
          }
  }
  Else{
      $VMHOST = (get-item "HKLM:\SOFTWARE\Microsoft\Virtual Machine\Guest\Parameters").GetValue("HostName")
      If($TestRegRoot -like "*False*"){
          New-ItemProperty -Path "HKLM:\System\Apps4Rent\" -Name Host -Value $VMHOST -PropertyType String -Force | Out-Null
      } Else {
          Set-ItemProperty -Path "HKLM:\System\Apps4Rent\" -Name Host -Value $VMHOST -PropertyType String -Force | Out-Null
      }
  }
}


    UpdateHost       #Updates the Hostname in Main RegKey