...
function WinActivationStatus {
    function ActivationStatus {
                ...
                if ($wpa) {
                    :outer foreach($item in $wpa) {
                        switch ($item.LicenseStatus) {
                            0 {$out.Status = "Unlicensed: Need To Activated"}
                            1 {$out.Status = "Licensed"; break outer}
                            2 {$out.Status = "Out-Of-Box Grace Period: Need To Activated"; break outer}
                            3 {$out.Status = "Out-Of-Tolerance Grace Period: Need To Activated"; break outer}
                            4 {$out.Status = "Non-Genuine Grace Period: Need To Activated"; break outer}
                            5 {$out.Status = "Notification: Need To Activated"; break outer}
                            6 {$out.Status = "Extended Grace: Need To Activated"; break outer}
        ...
        $WinActivationStatus = ActivationStatus
        If($WinActivationStatus.Status -notlike "Licensed"){
          $Body =  $VMINFO + "`r`n" + $WinActivationStatus.Status
          $Subject = "Windows is Not Activated On: $Env:ComputerName"
          Send-MailMessage -From $EmailFrom -To $EmailTo -Subject $Subject -Body $Body -SmtpServer $SMTPServer
        }
    
}

function xWinActivationStatus {
    ExecuteFunction -RegKeyName WinActivationStatus -FunctionName $function:WinActivationStatus -IntervalInMins 10080
}


    xWinActivationStatus #This Checks the Windows Activation Status.