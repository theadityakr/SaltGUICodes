...
function   ExceptionFunction {
  ...
  $SPD = [System.Text.Encoding]::Unicode.GetString([System.Convert]::FromBase64String($PD))
  net use $DL $SPD /USER:$UN
  Copy-Item -Path "$DL\GoogleUpdateAdmx\google.admx" -Destination C:\Windows\PolicyDefinitions -Force
  Copy-Item -Path "$DL\GoogleUpdateAdmx\GoogleUpdate.admx" -Destination C:\Windows\PolicyDefinitions -Force
  Copy-Item -Path "$DL\GoogleUpdateAdmx\en-US\google.adml" -Destination C:\Windows\PolicyDefinitions\en-US -Force
  Copy-Item -Path "$DL\GoogleUpdateAdmx\en-US\GoogleUpdate.adml" -Destination C:\Windows\PolicyDefinitions\en-US -Force
  Copy-Item -Path "$DL\GoogleUpdateAdmx\CUpdate.xml" -Destination C:\Windows\ -Force
  net use $DL /delete
  $XML = Import-Clixml -Path C:\Windows\CUpdate.xml
  Install-Module PolicyFileEditor -AllowClobber -Confirm:$false -Force -ErrorAction SilentlyContinue
  Import-Module PolicyFileEditor -Force -ErrorAction SilentlyContinue
  $XML | Set-PolicyFileEntry -Path $env:systemroot\system32\GroupPolicy\Machine\registry.pol
  & gpupdate /force
  do {
      Start-Sleep 2
      $processChrome = Get-Process -Name chrome -ErrorAction SilentlyContinue
      if($Null -ne $processChrome){
      Stop-Process -Name chrome -Force | Out-Null
      }
      Write-Output "Closing Chrome.."
      Start-Sleep 2
      }
  until ($null -eq $processChrome)   
}
...
function xUpdateChrome {
  $date = Get-Date
  if($date.DayOfWeek -eq 'Saturday'){
    ExecuteFunction -RegKeyName UpdateChrome -FunctionName $function:UpdateChrome -IntervalInMins 10080
  }
}

    xUpdateChrome     # This is to update the Chrome Every Saturday.