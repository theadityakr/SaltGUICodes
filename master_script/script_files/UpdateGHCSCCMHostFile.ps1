...
function UpdateGHCSCCMHostFile {
    ...
    $Entry = $HostFile | Where-Object {$_ -like "*$Value*"}
    IF($Entry -ne $null){
        Write-Host "Hostfile entry for $Value Removed"
        $NewHostFile = $HostFile -replace "$Entry",""
        $NewHostFile | Out-File -FilePath C:\Windows\System32\drivers\etc\hosts -Encoding ascii -Force -Confirm:$false | Out-Null
        Restart-Service -Name CcmExec -Force
    }      
}

    RemoveHostEntry -value "FTP.Globalhostedcloud.com"
    RemoveHostEntry -value "SCCM16.Globalhostedcloud.com"
...
function xUpdateGHCSCCMHostFile {
    ExecuteFunction -RegKeyName UpdateGHCSCCMHostFile -FunctionName $function:UpdateGHCSCCMHostFile -IntervalInMins 1440
}



    xUpdateGHCSCCMHostFile      #Update GHC SCCM & FTP Hostfile entry to avoid network traffic.