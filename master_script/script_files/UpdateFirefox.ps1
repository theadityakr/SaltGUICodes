...
function   ExceptionFunction {
 ...
 function UpdateFirefox{
  ...
  $FUS = "C:\Windows\System32\Sysprep\Apps4Rent\UpdateFirefox.ps1"
  If(Test-Path "C:\Windows\System32\Sysprep\Apps4Rent\UpdateFirefox.ps1"){
    $NFUS = Get-ChildItem -Path $DL -Recurse | Where-Object {$_.FullName -like '*UpdateFirefox*'} | Sort-Object -Property LastAccessTime -Descending | Select-Object -First 1
    IF((Compare-Object $(Get-Content $FUS) -DifferenceObject $(Get-Content $NFUS.FullName))){
        Copy-Item -Path "$DL\Apps4Rent\UpdateFirefox.ps1" -Destination "C:\Windows\System32\Sysprep\Apps4Rent\UpdateFirefox.ps1" -Force -Confirm:$false | Out-Null
    }
  } Else {
    Copy-Item -Path "$DL\Apps4Rent\UpdateFirefox.ps1" -Destination "C:\Windows\System32\Sysprep\Apps4Rent\UpdateFirefox.ps1" -Force -Confirm:$false | Out-Null
  }
  net use $DL /delete
  & "C:\Windows\System32\Sysprep\Apps4Rent\UpdateFirefox.ps1"
}
...
function xUpdateFirefox {
  $date = Get-Date
  if($date.DayOfWeek -eq 'Saturday'){
    ExecuteFunction -RegKeyName UpdateFirefox -FunctionName $function:UpdateFirefox -IntervalInMins 10080
  }
}

xUpdateFirefox    # This is to update the Firefox Every Saturday.