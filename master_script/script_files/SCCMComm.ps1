...
function SCCMComm {
    ...
    $minimumDate = ((Get-Date).AddDays(-8))
    if ($HWScanDate -le $minimumDate) {
        Set-Service Winmgmt -StartupType Automatic -Confirm:$false | Out-Null
        Set-Service CcmExec -StartupType Automatic -Confirm:$false | Out-Null
        Start-Service Winmgmt -Confirm:$false | Out-Null
        Start-Service CcmExec -Confirm:$false | Out-Null
        Restart-Service CcmExec -Confirm:$false | Out-Null
        $Body =  $VMINFO
        $Subject = "SCCM Server communication is broken on: $Env:ComputerName"
        ...
        Invoke-WmiMethod -Namespace 'root\ccm' -Class 'sms_client' -Name TriggerSchedule $HardwareInventory -ErrorAction SilentlyContinue | Out-Null 
        Invoke-WmiMethod -Namespace 'root\ccm' -Class 'sms_client' -Name TriggerSchedule $MachinePolicy -ErrorAction SilentlyContinue | Out-Null
...
function xSCCMComm {
    ExecuteFunction -RegKeyName SCCMComm -FunctionName $function:SCCMComm -IntervalInMins 1440
}


    xSCCMComm        # This verify the SCCM Client and Server communication based on H/W inventory Scans. Email Subject: SCCM Server communication is broken on: 