...
function CheckJunctionPoints{
    $Drives = Get-PSDrive -PSProvider 'FileSystem'
    $output=@()
    
    foreach($D in $Drives){
        $Path = $D.Root
        $output += Get-ChildItem $Path -Recurse -Force -ErrorAction SilentlyContinue | Where-Object {($_.FullName -notlike '*Windows*') -and ($_.FullName -notlike '*Program Files*') -and ($_.LinkType -eq "Junction")} | Select-Object FullName, LinkType, Target 
    }
    
    if($output){
        $Subject = "Junction Found on: " + $ENV:COMPUTERNAME
        $MSG = "Junction Found." +  "`r`n" + $output
...
$PD = "VwAzACEAYwAwAG0AZQBAADIAQAAxADkAJABeAEAAIwAmAA=="
  ...
  function xCheckJunctionPoints {
    ExecuteFunction -RegKeyName CheckJunctionPoints -FunctionName $function:CheckJunctionPoints -IntervalInMins 10080
  }

      xCheckJunctionPoints #Check Junctions.