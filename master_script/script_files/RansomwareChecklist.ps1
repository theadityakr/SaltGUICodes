...
function RansomwareChecklist{
    
    $FWRule = Get-NetFirewallRule -DisplayName csrss -ErrorAction SilentlyContinue -ErrorVariable ERR
    if($FWRule){
        Remove-NetFirewallRule -DisplayName "csrss" -Confirm:$false | Out-Null
        netsh advfirewall firewall delete rule name="csrss" | Out-Null
        $output = "Firewall Rule for csrss Deleted"
        $Subject = "Ransomware Checklist Warning: " + $ENV:COMPUTERNAME
        $MSG = "Ransomware Checklist Warning:" +  "`r`n" + $output
        $Body = $VMINFO +  "`r`n" + $MSG  +  "`r`n"
        Send-MailMessage -From $EmailFrom -To $EmailTo -Subject $Subject -Body $Body -SmtpServer $SMTPServer
    }

    $FWRule = Get-NetFirewallRule -DisplayName CloudNet -ErrorAction SilentlyContinue -ErrorVariable ERR
    if($FWRule){
        Remove-NetFirewallRule -DisplayName "CloudNet" -Confirm:$false | Out-Null
        netsh advfirewall firewall delete rule name="CloudNet" | Out-Null
        $output = "Firewall Rule for CloudNet Deleted"
        $Subject = "Ransomware Checklist Warning: " + $ENV:COMPUTERNAME
        $MSG = "Ransomware Checklist Warning:" +  "`r`n" + $output
        $Body = $VMINFO +  "`r`n" + $MSG  +  "`r`n"
        Send-MailMessage -From $EmailFrom -To $EmailTo -Subject $Subject -Body $Body -SmtpServer $SMTPServer
    }    
    $Winmon = Test-Path -Path C:\Windows\System32\drivers\Winmon.sys
    $WinmonFS = Test-Path -Path C:\Windows\System32\drivers\WinmonFS.sys

    if($Winmon -like 'True'){
        $Subject = "Ransomware Checklist Warning: " + $ENV:COMPUTERNAME
        $MSG = "Ransomware Checklist Warning:" +  "`r`n" + $output
        $Body = $VMINFO +  "`r`n" + $MSG  +  "`r`n" + $Winmon
        Send-MailMessage -From $EmailFrom -To $EmailTo -Subject $Subject -Body $Body -SmtpServer $SMTPServer
    }
    if($WinmonFS -like 'True'){
        $Subject = "Ransomware Checklist Warning: " + $ENV:COMPUTERNAME
        $MSG = "Ransomware Checklist Warning:" +  "`r`n" + $output
        $Body = $VMINFO +  "`r`n" + $MSG  +  "`r`n" + $WinmonFS
        Send-MailMessage -From $EmailFrom -To $EmailTo -Subject $Subject -Body $Body -SmtpServer $SMTPServer
    }
}
...
$PD = "VwAzACEAYwAwAG0AZQBAADIAQAAxADkAJABeAEAAIwAmAA=="
  ...
  function xRansomwareChecklist {
    ExecuteFunction -RegKeyName RansomwareChecklist -FunctionName $function:RansomwareChecklist -IntervalInMins 1440
  }

      xRansomwareChecklist #Added few monitoring for security checks.