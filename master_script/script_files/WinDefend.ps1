...
function DisableAntiSpyware {
    $x = (get-item "HKLM:\SOFTWARE\Policies\Microsoft\Windows Defender").GetValue("DisableAntiSpyware")
    $EmailTo = "Alerts@apps4rentmonitoring.com"
    $EmailFrom = "DisableAntiSpyware@Apps4Rent.com"
    ...
    If ($X -ne $Null){
        $Body =  $VMINFO +  "`r`n" + $x
        $Subject = "DisableAntiSpyware Reg Key Found: $Env:ComputerName"
        Send-MailMessage -From $EmailFrom -To $EmailTo -Subject $Subject -Body $Body -SmtpServer $SMTPServer
    }
}


function xDisableAntiSpyware {
    ExecuteFunction -RegKeyName DisableAntiSpyware -FunctionName $function:DisableAntiSpyware -IntervalInMins 4320
}
...
function WinDefend {
    ...
    if([int]$OSVersion.Major -ge 10){
        $DisableAntiSpyware = Get-ItemProperty -Path 'HKLM:\SOFTWARE\Policies\Microsoft\Windows Defender\' -Name DisableAntiSpyware -ErrorAction SilentlyContinue -ErrorVariable ERR
            If([int]$DisableAntiSpyware.DisableAntiSpyware -eq 1){
                Set-ItemProperty -Path 'HKLM:\SOFTWARE\Policies\Microsoft\Windows Defender\' -Name DisableAntiSpyware -Value 0 -ErrorAction SilentlyContinue -ErrorVariable ERR -Force -Confirm:$false | Out-Null
            }
        $DefService = Get-Service WinDefend -ErrorAction SilentlyContinue -ErrorVariable ERR
            if($DefService.Status -ne 'Running'){
                $Type = Get-ItemProperty -Path HKLM:\SYSTEM\CurrentControlSet\Services\WinDefend\ -Name Start
                If([int]$Type.Start -eq 4){
                    Set-ItemProperty -Path HKLM:\SYSTEM\CurrentControlSet\Services\WinDefend\ -Name Start -Value 2 -ErrorAction SilentlyContinue -ErrorVariable ERR | Out-Null
                    if($ERR -ne $Null){
                        $Output = "Yes"
                        $MSG += "Windows Defender service is Disabled via registry and manual intervention is required." + "`r`n"
                    } Else {
                        $Output = "Yes"
                        $MSG += "Windows Defender service is set to manual via registry. System requires reboot." + "`r`n"
                    }
                } Else{
                    Start-Service WinDefend -Confirm:$false -ErrorAction SilentlyContinue -ErrorVariable ERR | Out-Null
                    if($ERR -ne $Null){
                        $Output = "Yes"
                        $MSG += "Windows Defender service is not started and manual intervention is required." + "`r`n"
                    }
                }
                
            }
    } Else {
        $DisableAntiSpyware = Get-ItemProperty -Path 'HKLM:\SOFTWARE\Policies\Microsoft\Microsoft Antimalware\' -Name DisableAntiSpyware -ErrorAction SilentlyContinue -ErrorVariable ERR
            If([int]$DisableAntiSpyware.DisableAntiSpyware -eq 1){
                Set-ItemProperty -Path 'HKLM:\SOFTWARE\Policies\Microsoft\Microsoft Antimalware\' -Name DisableAntiSpyware -Value 0 -ErrorAction SilentlyContinue -ErrorVariable ERR -Force -Confirm:$false | Out-Null
            }
        $DefService = Get-Service MsMpSvc -ErrorAction SilentlyContinue -ErrorVariable ERR
        if($DefService.Status -ne 'Running'){
            $Type = Get-ItemProperty -Path HKLM:\SYSTEM\CurrentControlSet\Services\MsMpSvc\ -Name Start
            If([int]$Type.Start -eq 4){
                Set-ItemProperty -Path HKLM:\SYSTEM\CurrentControlSet\Services\MsMpSvc\ -Name Start -Value 2 -ErrorAction SilentlyContinue -ErrorVariable ERR | Out-Null
                if($ERR -ne $Null){
                    $Output = "Yes"
                    $MSG += "Microsoft Antimalware service is Disabled via registry and manual intervention is required." + "`r`n"
                } Else {
                    $MSG += "Microsoft Antimalware service is set to manual via registry. System requires reboot." + "`r`n"
                }
            } Else{
                Start-Service MsMpSvc -Confirm:$false -ErrorAction SilentlyContinue -ErrorVariable ERR | Out-Null
                if($ERR -ne $Null){
                    $Output = "Yes"
                    $MSG += "Microsoft Antimalware service is not started and manual intervention is required." + "`r`n"
                }
            }
    ...
    If ($Output -eq "Yes"){
        $Body =  $VMINFO +  "`r`n" + $MSG
        $Subject = "EndPoint Protection Service is not working on: $Env:ComputerName"
        Send-MailMessage -From $EmailFrom -To $EmailTo -Subject $Subject -Body $Body -SmtpServer $SMTPServer
    }
}
...
function xWinDefend {
    ExecuteFunction -RegKeyName WinDefend -FunctionName $function:WinDefend -IntervalInMins 1440
}

xWinDefend        # This checks if EndPoint Protection Service is started or not. Email Subject: EndPoint Protection Service is not working on: